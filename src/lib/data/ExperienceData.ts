export = {
  ...
  progress: function () {
    return me.getStat(12) === 99 ? 0 : (((me.getStat(13) as number - this.totalExp[me.getStat(12) as number]) / this.nextExp[me.getStat(12) as number]) * 100).toFixed(2);
  },

  // Total experience gained in current run
  gain: function () {
    return (me.getStat(13) as number - DataFile.getStats().experience);
  },

  // Percent experience gained in current run
  gainPercent: function () {
    return me.getStat(12) as number === 99 ? 0 : (this.gain() * 100 / this.nextExp[me.getStat(12) as number]).toFixed(6);
  },

  // Runs until next level
  runsToLevel: function () {
    return Math.round(((100 - this.progress()) / 100) * this.nextExp[me.getStat(12) as number] / this.gain());
  },

  // Total runs needed for next level (not counting current progress)
  totalRunsToLevel: function () {
    return Math.round(this.nextExp[me.getStat(12) as number] / this.gain());
  },

  // Total time till next level
  timeToLevel: function () {
    var tTLrawSeconds = (Math.floor((getTickCount() - me.gamestarttime) / 1000)),
  ...
  getGameTime: function () {
    ...
    if ((rawMinutes as any) <= 9) {
      rawMinutes = "0" + rawMinutes;
    }

    if ((rawSeconds as any) <= 9) {
