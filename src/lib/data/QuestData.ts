...
export class QuestInstance {
  ...
  public readonly reward: boolean
  public readonly areas: number[]
  public readonly bosses: number[]
  public readonly opensAreas: number[]
  public readonly prerequisites: number[]

  constructor(settings: Partial<Omit<Omit<QuestInstance, 'opensAreas'>, 'prerequisites'>>) {
    Object.assign(this, settings);

    this.opensAreas = QuestConsts.opensAreas.hasOwnProperty(this.name) ? QuestConsts.opensAreas[this.name] : [];
    this.prerequisites = QuestConsts.prerequisites.hasOwnProperty(this.name) ? QuestConsts.prerequisites[this.name] : [];
  }

}


export const MandatoryQuests = [
...
export function areasForQuest(q) {
  switch (q) {
...
export function bossForQuest(q) {
  switch (q) {
    case sdk.quests.SistersBurialGrounds:
...
for (let q = sdk.quests.SpokeToWarriv; q <= sdk.quests.SecretCowLevel; q++) {
  QuestData.push(new QuestInstance({
    index: q,
    name: Object.keys(sdk.quests).find(x => sdk.quests[x] === q),
    mandatory: MandatoryQuests.indexOf(q) > -1,
    reward: RewardedQuests.indexOf(q) > -1,
    areas: areasForQuest(q),
    bosses: bossForQuest(q),
  }));
}
