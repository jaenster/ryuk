...
function leftShift(val, shift) {
  let l;
  while (shift >= bpe) {
    l = Math.ceil((bitSize(val) + bpe - 1) / bpe) + 1;
    if (val.length < l)
      val = expand(val, l);
...
export class Rnd {
  ...
  constructor(seed) {
    let tmp = int2bigInt(666, 16, 0);
    tmp = leftShift(tmp, 32);
    this.val = add(tmp, str2bigInt(seed.toString(16), 16, 33));
  }

  roll() {
    let tmp = modInt(this.val, 0x100000000);
    const tmp2 = mult(str2bigInt(tmp.toString(16), 16, 33), int2bigInt(1791398085, 33, 0));
    rightShift(this.val, 32);
    let res = add(tmp2, this.val);
    const rescopy = dup(res);
...
export class Merc {
  ...
  public readonly hireling: any;
  public readonly typeid: number;
  public readonly subtype: number;
  public readonly skills: { name: string, lvl: number }[];
  ...
  public readonly attackrating: number;
  public readonly dmg_min: number;
  public readonly dmg_max: number;
  public readonly resists: number;

  constructor(name: number, seed: number) {
    this.id = name;
    ...
    const newseed = new Rnd(seed);

    newseed.roll();

    const index = (newseed.get()) % validMercTable.length;
    ...
    const difference = this.level - getMercInfo(index, 7);
...
export function mercPacket(pByte) {
  switch (pByte[0]) {
    case 0x4f:
